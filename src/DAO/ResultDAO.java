...
public class ResultDAO implements BaseDAO<ResultDTO, Integer> {
    ...
    public ResultDTO findByUserIdAndExCode(int userId, String exCode) {
        String query = "SELECT * FROM result WHERE userID = ? AND exCode = ?";
    ...
    public int getTakeExamTime(int userID, String testCode) {
        String query = "SELECT COUNT(*) AS count FROM result INNER JOIN exams " +
    ...
    public ResultDTO create(ResultDTO request) {
        String query = "INSERT INTO result (userID, exCode, rs_anwsers, rs_mark, rs_date) " +
                        "VALUES (?, ?, ?, ?, ?);";
